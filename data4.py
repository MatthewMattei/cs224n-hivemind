...
for cat in subcategories:
    ...
    if broader_cat in categories["niche"]:
        broader_cat = "niche"
    df['text'] = df.apply(combine_columns, axis=1)
    df = df[['text']]
    classify_df = pd.concat([classify_df, df], ignore_index=True)

classify_df.to_json(f'classify2.jsonl', orient='records', lines=True)