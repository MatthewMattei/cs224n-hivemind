...
def rephrase_input(organizer_model, organizer_tokenizer, user_input):
    inputs = organizer_tokenizer(user_input, return_tensors="pt").to(device)
    ...
    outputs = organizer_model.generate(
        input_ids=inputs['input_ids'],  # input sequence generated by organizer_tokenizer
        attention_mask=inputs['attention_mask'],
        max_length=512,     # can experiment with varying
        num_beams=5,    # can experiment with varying
        early_stopping=True     # can experiment with varying
    )
...
def choose_downstream_model(organizer_model, organizer_tokenizer, rephrased_input, downstream_models):
    inputs = organizer_tokenizer(rephrased_input, return_tensors="pt").to(device)
    scores = []
